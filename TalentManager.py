...
class TalentManager:
    ...
    talentDict: Dict[int, Talent] = {}
    grade_count = 4
    grade_prob = [0.889, 0.1, 0.01, 0.001]

    @staticmethod
    def load(config):
        TalentManager.talents = dict([(i, []) for i in range(TalentManager.grade_count)])
        TalentManager.talentDict = dict()

        for k in config.keys():
            t = Talent(config[k])
            TalentManager.talents[t.grade].append(t)
    ...
    def genTalents(self, count: int) -> Iterator[Talent]:
        ...
        for grade in range(TalentManager.grade_count - 1, -1, -1):
            count = counts[grade]
            n = len(TalentManager.talents[grade])
            if count > n:
                counts[grade - 1] += count - n
                count = n
            for talent in self._rnd.sample(TalentManager.talents[grade], k=count):
                yield talent
